...
'''
Before running the py file, run Pretreatment.py
'''


javaAPK = 0
skipjava = 0
max_tries = 1  # Maximum attempts
max_timeouts = 2  # Maximum timeout times
    ...
    for item in os.listdir(dir_path):
        ...
        if os.path.isdir(item_path): 
            ...
            try:
                ...
                isso = False
            '''
            if(not check_file_size(fullSOFilePath)): 
                smallso = True
            else:
                smallso = False
            '''
...
for name in os.listdir(apkspath):  
...
for i in range(len(folder_name)):
    try:
        ...
        try:
            ...
            print("Static analysis code execution time is %.6f" % duration," seconds") 
                    ...
                    for zip_info in apk_file.infolist():
                        if zip_info.filename.endswith('.so'):
                            try:
                                ...
                                os.chmod(apk_dir+'/'+zip_info.filename, 0o777)  
        ...
        while tries < max_tries:
            ...
            try:
                ...
                for i in range(6):
                    ...
                    apk_manifest = get_manifest(apkfilepath)  
                    main_activity = get_main_activity(apk_manifest)  
                    
                    start_app(packageName,main_activity) 
                    time.sleep(5)
