...
class ClassAnalysis:
    ...
    ClassAnalysis包含来自给定类的xref。
    它也被用来包装:class: ' ~androguard/core/bytecode/dvm/ClassDefItem '
    包含实际的类内容，如字节码。

    此外，外部类也会生成xrefs，显然外部类只显示XREF_FROM。

    :param classobj: class: ' ~androguard/core/bytecode/dvm/ClassDefItem '或:class: ' ExternalClass '
    """

    def __init__(self, classobj):
        # Automatically decide if the class is external or not
        self.external = isinstance(classobj, ExternalClass)
        #print(type(self))  ClassAnalysis
        self.orig_class = classobj
        #print(type(self.orig_class))  ClassDefItem 这个对象是属于DVM的
...
class Analysis:
    ...
    * fields (`FieldAnalysis`)
    ...
    分析对象
    该分析包含大量关于（多个）DalvikVMFormat对象的信息
    例如，功能是类、方法、字段和字符串之间的外部参照。
    另一部分是创建BasicBlocks，这对于Androiguard Decompiler的使用很重要。
    可以使用函数：meth:`add`添加多个DalvikVMFormat对象。
    创建外部参照的目的是：
    *类（`ClassAnalysis`）
    *方法（`MethodAnalysis`）
    *字符串（`StringAnalyis`）
    *字段（`FieldAnalysis`）
    在:class: ' ~androguard.core.bytecodes.apk.APK '之后，Analysis应该是唯一使用的对象。
    它将所有与Dalvik相关的函数封装到一个地方，而您仍然可以使用
    函数来自：class:`~androguard.core.bytecodes.dvm。DalvikVMFormat`和相关类。
    ：param可选[androguard.core.bytecodes.dvm.DalvikVMFormat]vm:inital DalvikVM格式对象（默认为无）
    """
    def __init__(self, vm=None):
        # Contains DalvikVMFormat objects  包含DalvikVMFormat对象
        self.vms = []
        # A dict of {classname: ClassAnalysis}, populated on add(vm)  {classname: ClassAnalysis}的字典，在add(vm)上填充
        
        self.classes = dict()
        # A dict of {string: StringAnalysis}, populated on create_xref()  {string: StringAnalysis}的字典，在create_xref()上填充    class如果不经过create_xref处理，那么数据不全？
        
        self.strings = dict()
        # A dict of {EncodedMethod: MethodAnalysis}, populated on add(vm)  {EncodedMethod: MethodAnalysis}的字典，在add(vm)上填充
        
        self.methods = dict()
        # Used to quickly look up methods  用于快速查找方法
        self.__method_hashes = dict()

        self.attributes = list()
        #新增attribute对象
        
        self.Fullmethods = list()
        #新增Fullmethods对象，用于记录methods的完整格式  类名.方法名
        if vm:
            self.add(vm)
    ...
    def get_Fullmethods(self):
        return self.Fullmethods
    ...
    def add(self, vm):
        ...
        # 这个步骤很容易是多线程的，因为在这个阶段对象之间没有依赖关系
        tic = time.time()  # 返回当前时间的时间戳
        #print("3333333333333")
        #print(len(vm.get_classes()))
        for current_class in vm.get_classes():
            #print("33333333333")
        
            #print(type(current_class))  ClassDefItem
            #为每个类建立一个ClassAnalysis对象，以字典的形式
            #调用的不是本地的dvm模块！ 
            
            self.classes[current_class.get_name()] = ClassAnalysis(current_class)
            #print(type(self.classes))  dict  内部形式为{类名：ClassAnalysis对象}
            #print(type(current_class.get_name()))  MUTF8String
            for method in current_class.get_methods():
                self.methods[method] = MethodAnalysis(vm, method)
                #print(self.methods[method]) #<analysis.MethodAnalysis Landroidx/appcompat/app/ActionBarDrawerToggle$Delegate;->setActionBarDescription(I)V [access_flags=public abstract] @ 0x0>
                #print(type(self.methods[method]))  #<class 'androguard.core.analysis.analysis.MethodAnalysis'>
                #print(self.methods[method].method.get_name())  #可以得到方法名 (不含类)
                #print(current_class.get_name())  #可以得到当前类名
                Fmethod = str(current_class.get_name()).strip(";") + '.' + str(self.methods[method].method.get_name())
                if Fmethod not in self.Fullmethods:
                    self.Fullmethods.append(Fmethod)
        ...
        log.info("Reading bytecode took : {:0d}min {:02d}s".format(*divmod(int(time.time() - tic), 60)))  #读取字节码花了x分钟x秒
    ...
    def create_xref(self):
        ...
        为分析中的所有类创建类、方法、字符串和字段交叉引用。

        如果使用多个DEX文件，则必须在添加所有DEX文件后调用此函数。如果在每个DEX文件之后调用该函数，那么它只会在第一次工作。
        
        这个函数在auto中调用了，先生成了dx对象，然后调用了create_xref函数
        """
        
        #这个函数只能调用1次，多次调用会导致错误
        ...
        # 多处理难以实现的一个原因是外部类和方法的创建。这必须同步，因为我们只有一个方法！
        
        #处理每个dvm对象
        for vm in self.vms:
            #处理每个dvm对象中的每个类
            for current_class in vm.get_classes():
                #对每个类调用_create_xref函数
        ...
        # 收集完所有信息后，我们应该向每个MethodAnalysis添加字段和字符串外部参照 
    ...
    def _create_xref(self, current_class):
        ...
        *       for field manipulation
        ...
        '''
        在create_xref函数中，调用了_create_xref，具体的工作都在_create_xref中完成
        
        对每个dvm中的每个类都调用_create_xref
        即，对apk中所有的类都调用_create_xref
        ...
        for current_method in current_class.get_methods():
            ...
            #print(type(current_class))   #ClassDefItem
            #print(type(current_method))  #EncodedMethod
            
            #获取到一个方法
            cur_meth = self.get_method(current_method)
            #print(type(cur_meth))  #MethodAnalysis
            #获取这个方法的所属类
            cur_cls = self.classes[cur_cls_name]
            #print(type(cur_cls))   #ClassAnalysis
            ...
            for off, instruction in current_method.get_instructions_idx():
                ...
                #print(instruction)  smali语句 例如  move-result-object v2        invoke-direct v0, Ljava/lang/Object;-><init>()V
                
                op_value = instruction.get_op_value()
                #print(type(op_value))  #int
                #print(op_value)

                

                # 1) check for class calls: const-class (0x1c), new-instance (0x22)
                # 1) 检查类调用:const-class (0x1c)， new-instance (0x22)
                if op_value in [0x1c, 0x22]:
                    idx_type = instruction.get_ref_kind()
                    #print(type(idx_type)) #int
                    # type_info is the string like 'Ljava/lang/Object;'  type_info是'Ljava/lang/Object;'这样的字符串
                    #print("1111")
                    try:
                    ...
                    #print("1111")
                    if type_info[0] != b'L':
                        # Need to make sure, that we get class types and not other types   需要确保，我们得到的是类类型，而不是其他类型
                        continue

                    if type_info == cur_cls_name:
                        # FIXME: effectively ignoring calls to itself - do we want that?  修复：有效地忽略对自身的调用-我们想要这样吗？
                        continue

                    if type_info not in self.classes:
                        # Create new external class  创建新的外部类
                ...
                # 2) 检查方法调用:invoke-* (0x6e…0x72)， invoke-xxx/range (0x74…0 x78)
                    ...
                    if class_info[0] != b'L':
                        ...
                        #需要确保，我们得到的是类类型，而不是其他类型。如果使用了另一种类型，比如int，我们就直接跳过它。
                        continue

                    # Resolve the second MethodAnalysis  解决第二个方法分析  
                    # _resolve_method在这个函数内，再一次的将外部类加入到self.classes中  因此 invoke-static可以被找到！
                    #进入_resolve_method后直接判断class_info在不在self.classes中，不在的话就把他加进去
                ...
                # 3) 检查字符串的使用情况:const-string (0x1a)， const-string/jumbo (0x1b)
                ...
                # 也许我们应该在这里添加一个步骤3a)，检查所有的const字段。然后你可以xref for integer等等!
                # But:这不起作用，因为const字段通常在内部优化为const调用…
                

                # 4) check for field usage: i*op (0x52 ... 0x5f), s*op (0x60 ... 0x6d)
                # 4) 检查字段使用情况:i*op (0x52…0x5f)， s*op (0x60…0 x6d)
                elif 0x52 <= op_value <= 0x6d:
                    idx_field = instruction.get_ref_kind()
                    field_info = instruction.cm.vm.get_cm_field(idx_field)
                    #field_info 中有build类！
                    infolen = len(field_info)
                    
                    for i in range(infolen):
                        if field_info[i][0] == b'L' and field_info[i][-1] == b';':
                            if field_info[i] not in self.classes:
                                self.classes[field_info[i]] = ClassAnalysis(ExternalClass(field_info[i]))
                    
                    #属性 attribute
                    fAttribute = str(field_info[0]).strip(';') + '.' + str(field_info[-1])
                    #print(type(fAttribute))  #str
                    
                    if fAttribute not in self.attributes:
                        self.attributes.append(fAttribute)
                    ...
                    if (0x52 <= op_value <= 0x58) or (0x60 <= op_value <= 0x66):
                        # read access to a field  读取字段的访问权限
                        # 在这个下面增加部分代码  将build类放入self中
                        # 0x22: [Instruction21c, ["new-instance", Kind.TYPE]], new-instance vAA, type@BBBB
                        # 0x62: [Instruction21c, ["sget-object", Kind.FIELD]]  sstaticop vAA, field@BBBB
                        # 两者都是Instruction21c类型的，获取对应类的方式也应该相同  区别在于一个是kind.Type  一个是kind.field
                        idx_meth = instruction.get_ref_kind()
                        #idx_meth 中没有-1，但是还是出现了无效的type
                        
                        #if op_value == 0x62:
                            #out = instruction.get_output()
                            #idx_meth = instruction.get_ref_kind()   # return self.BBBB
                            #type_info = instruction.cm.vm.get_cm_type(idx_meth)
                            #print(idx_meth)  #超过了4259 type_ids_size的大小是4259
                            #type_info = instruction.cm.vm.get_cm_type(4500)
                            #print(type_info)  AG:ITI: invalid type  若超过4259则会报错
                            #print(type_info) Ldalvik/annotation/MemberClasses; 不超过4295则正常显示类名
                            #struct field_id_list dex_field_ids    大小是29571 fields
                            #if idx_meth >= 29571:
                            #    print("1111111111111111111111")
                            #idx_meth都在29571的范围之内
                            
                                                   
                        
                        #idx_meth  # int 返回的是instruction21c.BBBB    sstaticop vAA, field@BBBB
                        
                        #print(instruction.cm) ClassManager
                        #模仿第一部分  创建新的外部类
                        type_info = instruction.cm.vm.get_cm_type(idx_meth)
                        
                        if type_info[0] == b'L' and type_info[-1] == b';':
                            if type_info not in self.classes:
                                self.classes[type_info] = ClassAnalysis(ExternalClass(type_info))
                        
                        
                        #模仿第二部分 从smali语句中增加类
                        method_info = instruction.cm.vm.get_cm_method(idx_meth)    #关键在于这个函数    get_cm_method 通过使用索引获得特定的方法
                        #get_cm_method 返回一个三元组，分别是某个方法对应的类名，此方法的方法名，此方法的原型
                        #print(method_info)  [Landroid/content/ComponentName;, equals, [(Ljava/lang/Object;), Z]]
                        #print("3333")
                        try:
                            class_info = method_info[0].lstrip(b'[')
                        except Exception as e:
                            ...
                            continue
                        #print("3333")
                        if class_info[0] == b'L' and class_info[-1] == b';':
                            if class_info not in self.classes:
                                self.classes[class_info] = ClassAnalysis(ExternalClass(class_info))
                        
                        #第二部分只到这里,后面是我自己加上去的，但他们是有效的
                        #遍历返回的原型列表，加入self.classes中
                        lenth = len(method_info[2])
                        for i in range(lenth):
                            #print("444444")
                            try:
                                class_info = method_info[2][i].lstrip(b'[(')
                            except Exception as e:
                                #print("error: ",e)
                                continue
                            #print("444444")
                            if class_info[0] == b'L' and class_info[-1] == b';' :
                                if class_info not in self.classes:
                                    theid = class_info.find(";")
                                    class_info = class_info[0:theid+1]
    ...
    def _resolve_method(self, class_name, method_name, method_descriptor):
        ...
        解析方法并返回MethodAnalysis。
        是否会自动创建ExternalMethods，如果不能解析并添加到ClassAnalysis等

        : param str class_name:
        : param str method_name:
        method_descriptor:有参数和返回类型的元组，即['(I Z)'， 'V']
        返回:
        : rtype MethodAnalysis
        """
        ...
        if m_hash not in self.__method_hashes:
            ...
            if class_name not in self.classes:
                # External class? no problem!  外部类?没问题!
            ...
            Fmethod = str(class_name).strip(";") + '.' + str(self.methods[meth].method.get_name())
            if Fmethod not in self.Fullmethods:
                self.Fullmethods.append(Fmethod)
