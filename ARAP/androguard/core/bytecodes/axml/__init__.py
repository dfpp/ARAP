...
class AXMLParser:
    ...
    AXMLParser读取AXML文件中的所有块，并实现一个状态机来返回关于当前块的信息，然后可以由:class: ' ~AXMLPrinter '读取这些信息。
    ...
    一个AXML文件是一个包含多个数据块的文件，由' ResChunk_header '定义。没有真正的文件魔法，但是由于第一个头的大小是固定的，并且`ResChunk_header`的`type`被设置为`RES_XML_TYPE`，一个文件通常以`0x03000800`开始。
但是有几个例子中，' type '被设置为其他的东西，可能是为了欺骗解析器。
    ...
    通常情况下，AXMLParser在循环中使用，如果' m_event '设置为' END_DOCUMENT '则循环终止。你可以使用' next() '函数来获取下一个块。注意，不是所有的块类型都是迭代器生成的!有些块只在AXMLParser中处理。
    
    The parser will set `is_valid()` to False if it parses something not valid.
    Messages what is wrong are logged.
    
    如果解析器解析的是无效的内容，它将把' is_valid() '设置为False。
    错误的消息被记录下来。
...
class AXMLPrinter:
    ...
    将AXML文件转换为lxml ElementTree的转换器，它可以很容易地转换为XML。
    
    A Reference Implementation can be found at http://androidxref.com/9.0.0_r3/xref/frameworks/base/tools/aapt/XMLNode.cpp
    参考实现可以在http://androidxref.com/9.0.0_r3/xref/frameworks/base/tools/aapt/XMLNode.cpp找到
    """
    __charrange = None
    __replacement = None
    ...
    def get_xml_obj(self):
        ...
        获取XML作为ElementTree对象

        :返回::类:“lxml.etree.Element”
        """
        return self.root
    ...
    def _fix_name(self, prefix, name):
        ...
        Then, the tag name will usually look like 'android:foobar'. #然后，标签名通常看起来像“android:foobar”。
