...
# TODO: have some more generic magic...   有一些更普通的魔法……
...
def readuleb128(cm, buff):
    ...
    在缓冲区的当前位置读取一个无符号LEB128  LEB128是一种压缩算法  是DalVik中使用的一种编码算法

    :param buff:一个类似文件的对象
    :返回:解码的无符号LEB128
    """
...
def determineNext(i, cur_idx, m):
    ...
    确定：class:`EncodedMethod`字节码内的下一个偏移量。
    偏移量以从方法开始的字节数计算。
    注意，字节码内的偏移量用16位单位表示，但此方法返回实际字节！
    操作码内的偏移量从操作码的开头开始计算。
    返回的类型是一个列表，因为分支操作码将有多个路径。
    `if`和`switch`操作码将返回列表中的多个项，而
    `throw`、`return`和`goto`操作码将始终返回长度为1的列表。
    偏移量-1表示方法已退出，例如通过“throw”或“return”退出。
    如果输入的操作码不是分支或跳转，则返回空列表。
    ：param指令i：当前指令
    ：param int cur_idx：指令索引
    ：param EncodedMethod m：当前方法
    ：返回：
    ：rtype：列表
    """
...
class HeaderItem:
    ...
    这个类可以解析dex文件的header_item。
    
    header_item 是dex文件结构中的第一大项
    
    执行几个检查来检测这是否不是一个header_item。
    此外，为了检测文件损坏，还要计算文件的Adler32校验和。
    :表示header_item的buff对象的字符串
    :类型androguard.core. byteccode . buffhandle buff: buff对象
    :参数cm: ClassManager对象
    : cm型::类:“ClassManager”
    """
    ...
    def __init__(self, size, buff, cm):
        self.CM = cm
        #print(type(self.CM))  #ClassManager

        self.offset = buff.get_idx()
        #print(type(buff)) #DalvikVMFormat
        #print(self.offset) #0
        ...
        # stuff...   Q实际上是错的，但我们不会在这里更改它，也不会打开我们自己的东西。。。
        self.magic, \
        self.checksum, \
        self.signature, \
        self.file_size, \
        ...
        #print(self.magic)      #b'dex\n035\x00'  √
        #print(self.checksum)   #941749200          √
        #print(self.signature)  #b'_\xb1`m\xa5<\xfa\xea(b\x92\x95~\x06\x1c\x81\x98\x94M\x1e' √
        #print(self.file_size)  #4279240   √
        #print(self.header_size) #112   √
        #print(endian_tag)       #305419896 √
        #print(self.link_size)   #0 √
        #print(self.link_off)   #0  √   
        #print(self.map_off)  #4279032  √
        #print(self.string_ids_size) #34251     √
        #print(self.string_ids_off)   #112      √
        #print(self.type_ids_size)  #4259       √
        #print(self.type_ids_off)   #137116     √
        #print(self.proto_ids_size) #6644       √
        #print(self.proto_ids_off)   #154152    √
        #print(self.field_ids_size)   #29517    √
        #print(self.field_ids_off)   #233880    √
        #print(self.method_ids_size) #33643     √
        #print(self.method_ids_off)  #470448    √
        #print(self.class_defs_size) #3295      √
        #print(self.class_defs_off)  #739592    √
        #print(self.data_size)       #3434208   √
        #print(self.data_off)        #845032    √
        

        # possible dex or dey:  #从魔术头检查dex文件的有效性
        ...
        #解析dex的第一大项header_item  并根据解析的值，对dex的有效性做出判断
...
class TypeHIdItem:
    ...
    :param cm: a ClassManager object
    :type cm: :class:`ClassManager`
    这个类可以解析dex文件的type_id_item列表

    :一个表示type_id_item列表的buff对象的字符串
    :类型buff: buff对象
    :参数cm: ClassManager对象
    : cm型::类:“ClassManager”
...
class MethodIdItem:
    ...
    这个类可以解析dex文件的method_id_item

    :一个表示method_id_item的buff对象的字符串
    :类型buff: buff对象
    :参数cm: ClassManager对象
    : cm型::类:“ClassManager”
    """
...
class EncodedMethod:
    ...
    这个类可以解析dex文件的encoded_method

    :一个表示encoded_method的buff对象的字符串
    :类型buff: buff对象
    :参数cm: ClassManager对象
    : cm型::类:“ClassManager”
    """
    ...
    def reload(self):
        ...
        '''
        这样做可能会更优雅:
        # get_method返回一个包含已解析类型的数组。
        但是，例如，为了计算参数的数量，我们现在需要分割字符串。
        这是非常繁琐的，如果我们返回类型id并在这里解析它们就可以避免。
        '''
        if v and len(v) >= 3:
            self.class_name = v[0]
            self.name = v[1]
    ...
    def get_length(self):
        ...
        返回与方法关联的代码对象
        ：rtype:：class:`DalvikCode`对象，如果没有“代码”，则为“无”
        """
    ...
    def get_instructions_idx(self):
        ...
        遍历该方法的所有指令，但也返回当前索引。
        这与每次使用：meth:`get_instructions`并将指令长度添加到变量中是一样的。
        ：返回：
        ：rtype：迭代器[（int，指令）]
        """
        #print(type(self))   #EncodedMethod
        if self.get_code() is None:
            #print("1111111111")
            return []
        #print("22222222")
...
class ClassDefItem:
    ...
    这个类可以解析dex文件的class_def_item

    :一个表示class_def_item的buff对象的字符串
    :类型buff: buff对象
    :参数cm: ClassManager对象
    : cm型::类:“ClassManager”
    """
...
class ClassHDefItem:
    ...
    :param cm: a ClassManager object
    :type cm: :class:`ClassManager`
    此类可以解析索引文件的class_def_item列表
    ：param buff：表示class_def_item列表的buff对象的字符串
    ：type buff：缓冲对象
    ：param cm:ClassManager对象
    ：键入cm:：class:`ClassManager`
    """
...
class Instruction:
    ...
    它既可以处理普通指令，也可以处理优化指令。

    . .警告::
    关于优化操作码的文档不多!
    因此，它依赖于反向工程规范!

    更多关于指令格式的信息可以在官方文档中找到:
    https://source.android.com/devices/tech/dalvik/instruction-formats.html

    . .警告::
    存储在指令中的值在这个阶段已经被解释了。

    Dalvik VM有8个操作码来创建恒定的整数值。
    32位值有4个变体，64位有4个变体。
    如果需要浮点数，你必须使用转换操作码，如:code: ' int-to-float '，:code: ' int-to-double '或使用:code: ' long '的变体。

    Androguard将始终显示值，因为它们在操作码中使用，也扩展符号和shift值!
    举个例子:opcode: ' const/high16 '可以用来创建下16位全为零的常量值。
    在这种情况下，androguard将bytecode:code: ' 1500 CD AB '处理为:code: ' const/high16 v0, 0xABCD0000 '。
    对于符号扩展，这里什么都不做，因为它只影响位表示
    在虚拟机中。由于androguard在内部解析值并使用python类型，所以我们不受特定大小的限制。
    ...
    def get_kind(self):
        ...
        返回指令的“kind”参数
        这是参数的类型，即您必须在哪种表中查找ClassManager中的参数
        ：rtype:整数
        """
        if self.OP >= 0xf2ff:
...
class Instruction21c(Instruction):
    ...
    def __init__(self, cm, buff):
        ...
        #print(cm.packer["BBH"].unpack(buff[:self.length]))
        #print(cm.packer["BBH"])
        #print(type(cm.packer["BBH"]))  # <class 'Struct'>
        #print(self.OP)  int 
        #print(self.AA) int
        #self.BBBB   int
    def get_output(self, idx=-1):
    ...
    #: rtype:字符串
        kind = get_kind(self.cm, self.get_kind(), self.BBBB)
        #print(self.get_kind())  Kind.FIELD  不是具体值
        #print(kind)   # Landroidx/appcompat/R$styleable;->ActionBar [I
...
DALVIK_OPCODES_FORMAT = {
    ...
    # 修理我：Kinds是怎么回事？还需要在get_kinds和操作码中实现
...
def get_instruction(cm, op_value, buff):
    ...
    :rtype: Instruction
    为给定的操作码返回:class: ' Instruction '

    cm: ClassManager传播到指令
    :param int op_value:指令的整数值
    :param bytearray buff:从指令开始的字节码
    :返回解析后的指令
    : rtype:指令
    """
    try:
        # DALVIK_OPCODES_FORMAT内部是： 0x00: [Instruction10x, ["nop"]],
...
class MapItem:
    def __init__(self, buff, cm):
        ...
        #TypeMapItem 是一个枚举类型，传入不同的int值，返回不同的Type
        '''
        HEADER_ITEM = 0x0
        STRING_ID_ITEM = 0x1
        TYPE_ID_ITEM = 0x2
...
class ClassManager:
    ...
    此类用于根据偏移量或索引访问dex格式的所有元素（字符串、类型、协议…）。
    """

    def __init__(self, vm):
        """
        :param DalvikVMFormat vm: the VM to create a ClassManager for
        param DalvikVMFormat vm：要为其创建ClassManager的vm
        """
        self.vm = vm
        self.buff = vm
        ...
        if self.vm:
            # print("11111111111111")  #这里是要执行的
            # 判断VM的类型是ODEX还是DEX   self.vm.get_format_type() == "ODEX"
            self.odex_format = self.vm.get_format_type() == "ODEX"
            #self.odex_format == False 即，这是个DEX类型
            #print(self.odex_format)
            #print(self.vm.get_format_type())
        else:
...
#第二个函数 unpack 将字节字符串解包成为变量。
    ...
    def get_odex_format(self):
        """Returns True if the underlying VM is ODEX  如果基础VM是ODEX，则返回True"""
    ...
    def set_decompiler(self, decompiler):
        #print(decompiler)  None
        #这里默认传的是None
        self.decompiler_ob = decompiler
        # print(type(self)) ClassManager
        #print(self.decompiler_ob) None
    ...
    def get_string(self, idx):
        ...
        从索引为' idx '的字符串表返回一个字符串

        如果字符串被钩住，则返回钩住的字符串。

        :param int idx:字符串部分的索引
        """
    ...
    def get_type_ref(self, idx):
        ...
        该方法类似于:meth: ' get_type '，但不解析字符串，而是将ID返回到字符串部分。

        如果没有找到类型IDX，则返回-1。
        """
        #print(type(self.__manage_item[TypeMapItem.TYPE_ID_ITEM]))  TypeHIdItem
        #mytype = self.__manage_item[TypeMapItem.TYPE_ID_ITEM].get_type()
        #print(len(mytype))  #4259
    ...
    def get_method(self, idx):
        #print(type(self.get_method_ref(idx)))  #MethodIdItem
        #print(type(self.get_method_ref(idx).get_list()))  list
        #print(self.get_method_ref(idx).get_list())  [Landroidx/activity/result/IntentSenderRequest;, <init>, [(Landroid/os/Parcel;), V]]
...
class MapList:
    ...
    此类可以解析dex格式的“map_list”

    https://source.android.com/devices/tech/dalvik/dex-format#map-list
    
    MapList
    该列表包含关于 header_item 的一些冗余信息，但这些冗余信息存在的目的是提供一种用于遍历整个文件的简单方式
    MapList   size   uint               列表的大小（以条目数表示）
              list   map_item[size]     列表的元素
    
    
    map_item
    type    ushort   项的类型
    unused  ushort   未使用（默认值为0）
    size    uint     在指定偏移量处找到的项数量
    offset  uint     从文件开头到相关项的偏移量     (这一项的存在保证了遍历整个文件的简便性)
    ...
    def __init__(self, cm, off, buff):
        ...
        self.size, = cm.packer["I"].unpack(buff.read(4))
        #print(self.size)  17  也就是说map_item[size]中，共有17个map_item   具体可以从010中查看
        ...
        #print(len(self.map_item))
        #self.map_item中的每一项都是一个Mapitem对象
        load_order = TypeMapItem.determine_load_order()
        #print(type(load_order))  #dict
        #print(load_order)
        
        '''
        {<TypeMapItem.HEADER_ITEM: 0>: 0, 
        <TypeMapItem.MAP_LIST: 4096>: 1, 
        <TypeMapItem.STRING_DATA_ITEM: 8194>: 2, 
        <TypeMapItem.STRING_ID_ITEM: 1>: 3, 
        <TypeMapItem.TYPE_ID_ITEM: 2>: 4, 
        <TypeMapItem.FIELD_ID_ITEM: 4>: 5, 
        <TypeMapItem.TYPE_LIST: 4097>: 6, 
        <TypeMapItem.PROTO_ID_ITEM: 3>: 7, 
        <TypeMapItem.METHOD_ID_ITEM: 5>: 8, 
        <TypeMapItem.METHOD_HANDLE_ITEM: 8>: 9, 
        <TypeMapItem.CALL_SITE_ITEM: 7>: 10, 
        <TypeMapItem.CLASS_DATA_ITEM: 8192>: 11, 
        <TypeMapItem.DEBUG_INFO_ITEM: 8195>: 12, 
        <TypeMapItem.CODE_ITEM: 8193>: 13, 
        <TypeMapItem.ANNOTATION_ITEM: 8196>: 14, 
        <TypeMapItem.ANNOTATION_SET_ITEM: 4099>: 15, 
        <TypeMapItem.ANNOTATION_SET_REF_LIST: 4098>: 16, 
        <TypeMapItem.ENCODED_ARRAY_ITEM: 8197>: 17, 
        <TypeMapItem.ANNOTATIONS_DIRECTORY_ITEM: 8198>: 18, 
        <TypeMapItem.CLASS_DEF_ITEM: 6>: 19}
        '''
...
class DalvikVMFormat(bytecode.BuffHandle):
    ...
    example::
        ...
        这个类可以解析类。Android应用程序（APK）的dex文件。
        ：param buff：表示类/dex文件的字符串
        ：param decompiler：关联反编译器对象以显示java源代码
        ：type buff：字节
        ：type反编译器：对象
        例子：：
        d=DalvikVMFormat（read("classes.dex")）
    ...
    def __init__(self, buff, decompiler=None, config=None, using_api=None):
        ...
        # 为了允许传递apk对象==>，我们不需要另外传递目标版本
        # isinstance 判断buff是否是APK类型
        # print(type(buff))  # <class 'bytes'>  因为传过来的是RAW data
        # print(using_api)
        if isinstance(buff, APK):
            self.api_version = buff.get_target_sdk_version()
            
            buff = buff.get_dex()  # getting dex from APK file
        elif using_api:  #using_api == None
        ...
        #print(self.CM.decompiler_ob)  #None
        #print(type(self.CM.decompiler_ob))  #None

        self._preload(buff)  #什么也不做 这个函数内部是pass
        self._load(buff)
    ...
    def _load(self, buff):
        ...
        #每一个vm都对应一个ClassManager
        
        
        #print(type(self.header))  #HeaderItem
        
        #HeaderItem -> MapList -> classes/methods/fields/codes/strings/debug
        #HeaderItem中完成对dex头部的解析，并简单判断dex是否有效

        if self.header.map_off == 0:
            # TODO check if the header specifies items but does not have a map                              TODO检查标头是否指定了项，但没有映射
            ...
            # print(type(self.classes))  ClassHDefItem
    ...
    def get_classes(self):
        ...
        返回所有类
        ：rtype:class:`ClassDefItem`对象列表
        """
        #print("2222222222222222222")
        if self.classes:
            #print(type(self.classes.class_def))  #list
            #print("111111111111")
