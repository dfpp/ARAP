...
class PathPlanner:
    ...
    def __init__(self, package, atg_json, deeplinks_json):
        ...
        # 检查 deeplinks 中是否存在 package
        package_deeplinks = self.deeplinks.get(self.package)
        if not package_deeplinks:
            print(f"Warning: No deeplinks found for package '{self.package}'. Using empty defaults.")
            package_deeplinks = {}

        self.total_activity = len(package_deeplinks.keys())
        for activity in package_deeplinks.keys():
            self.visited_map.setdefault(activity, [False, False])
    ...
    def get_visited_rate(self):
        ...
        if self.total_activity == 0:
            #print("Warning: Total activity count is zero. Returning visited rate as 0.")
            return 0.0
